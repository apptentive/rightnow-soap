...
class RightNow::Client
  ...
  def create(object)
    ...
    if (error = response_items[0][:request_error_fault])
      ...
      case message
      when /invalid/i
        raise RightNow::InvalidObjectError, message
      when %r{Cannot save/create}
        raise RightNow::DuplicateObjectError, message
      else
        raise RightNow::Error, message
      end
    end
    ...
    raise RightNow::InvalidObjectError, ex.message
  end
